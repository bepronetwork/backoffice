...
class Won extends PureComponent {
    ...
    projectData = async (props) => {
        ...
        if (!_.isEmpty(data.data)) {
            const bets = this.getAllBets(data.data);

            this.setState({
                bets: bets,
                periodicity: periodicity,
                percentage: bets.percentage_won
            })
        } else {
            this.setState({
                bets: [],
    ...
    render() {  
        ...
        const { percentage, periodicity } = this.state;

        const percentageWon = 100-Numbers.toFloat(percentage*100);
                    ...
                    <Skeleton variant="rect" height={29} style={{ marginTop: 10, marginBottom: 10 }}/> 
                        ...
                        <div className="dashboard__visitors-chart">
                            <p className="dashboard__visitors-chart-number-second" style={
                                {color : percentageWon >= 0 ? '#76d076' : '#646777'}
                            }>
                                <AnimationNumber decimals={2} number={percentageWon}/>
                                <span className="dashboard__visitors-chart-title"> % </span>
                            </p>
                        </div>)}
                        <div className="dashboard__visitors-chart">
                            <p className="dashboard__visitors-chart-title"> Won Bets <span> {periodicity} </span></p>
                        </div>
                    </CardBody>
                </Card>
            </Col>
