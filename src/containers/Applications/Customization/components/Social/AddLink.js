...
class AddLink extends React.Component {
    ...
    projectData = async (props) => {
        ...
        this.setState({ links: !_.isEmpty(links) ? links : [] })
    }
    ...
    onChangeNewName = ({ value }) => {
        this.setState({ newName: value ? value : "" })
    }

    onChangeNewLink = ({ value }) => {
        this.setState({ newLink: value ? value : "" })
    }
    ...
    addNewLink = () => {
        const { newName, newLink, newImage, isSVG } = this.state;
        const { setLinks, links } = this.props;
        
        const newLinkObj = { name: newName, href: newLink, image_url: newImage, _id: Math.random().toString(36).substr(2, 9), isSVG: isSVG }
    ...
    render() {
        ...
        return (
            <Col md={3} style={{ minWidth: 230, margin: 10 }}>
                <LinkCard>
                    <LinkCardContent>
                        { newImage ? 
                        <>  
                        ...
                        <AddLinkButton disabled={locked || hasEmptyValues} onClick={() => this.addNewLink()}>
                            <PlusIcon/> Add link
                        </AddLinkButton>
