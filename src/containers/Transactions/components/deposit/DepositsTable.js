...
const fromDatabasetoTable = (data, currencies) => {
	return data.map( (data) => {
        return {
            id :  data._id,
            currency: currencies.find(c => c._id === data.currency),
...
class DepositsTable extends React.Component {
    ...
    constructor(props){
        ...
        this.state = {
            ...
            orderBy: 'id',
            selected: [],
            data: props.data ? fromDatabasetoTable(props.data, props.currencies) : [],
            page: 0,
            isLoading : {},
            rowsPerPage: 5,
            ...defaultProps
    ...
    render() {
        ...
        const csvData = data.map(row => ({...row, currency: row.currency.ticker, creation_date: moment(row.creation_date).format("lll")}));
                    ...
                    {stableSort(data, getSorting(order, orderBy))
                        ...
                        return (
                            ...
                            >
                                ...
                                <StyledTableCell align="left"> 
                                    <p className='text-small'>
                                        {parseFloat(n.amount).toFixed(6)} {n.currency.ticker}
                                    </p>
                                </StyledTableCell>
